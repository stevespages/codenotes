...
<body>
  <div class="content">
    ...
    <p>
      Looking at the HTML tells us that the first element that is a parent of both the hamburger button and the navigation list is the <em>div</em> element with <em>id="nav"</em> (appended with <em>-n</em> in our examples in order that the CSS can distinguish between them). So, let us apply the <em>:focus-within</em> pseudo class to this <em>div</em> element and select its child element with the class name of <em>top-level-list</em>. Note that the <em>+</em> has now gone because we are selecting the <em>top-level-list</em> element that is a child of the focussed element not a sibling of it as before. The CSS now looks like this:
    </p>
    <pre><code>
      #nav-3 .top-level-list {
        display: none;
